...
a <- df$reg[[1]]
environment(a)
a$call$data <- df$data[[1]]

  update(a, y ~ . - lag.quarterly.revenue)
  data_frame(data = list(freeny)) %>%
    mutate(reg = map(data, ~ model_felm <- felm(log(gsp) ~ log(pcap) + log(pc) + log(emp) + unemp |state|0|state, data = Produc)),
           dec = map(reg, ~dec_covar(object = ., var_main = "lag.quarterly.revenue"))) %>% unnest(dec)
